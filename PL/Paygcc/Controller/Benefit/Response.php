...
class Response extends Benefit
    implements CsrfAwareActionInterface, HttpPostActionInterface, HttpGetActionInterface
{
    ...
    public function execute()
    {
       ...
       $payGCCOrderId = $this->_benefitModel->getPayGCCOrderId($this->_quote->getReservedOrderId());
       $response = $this->_benefitModel->getPayGCCOrderDetails($payGCCOrderId);
       $paymentInfo = $this->_paygccHelper->getTransactionDetails($response);

        if($this->_benefitModel->getConfigData('debug')) {
            $this->_plLogger->debug("benefit response>>>>>>>".print_r($response,1));
            $this->_plLogger->debug("beneift payment Info>>>>>>>".print_r($paymentInfo,1));
        }
        if(isset($paymentInfo['result']) && ($paymentInfo['result'] == 'CAPTURED' || $paymentInfo['result'] == 'SUCCESS')) {
            $this->_benefitCheckout->place($paymentInfo);
            ...
            $order = $this->_benefitCheckout->getOrder();
            if ($order) {
                $this->_getCheckoutSession()
