...
class Benefit extends \Magento\Payment\Model\Method\AbstractMethod
{
    ...
    protected $_infoBlockType = 'PL\Paygcc\Block\Benefit\Info';

    protected $_formBlockType = 'PL\Paygcc\Block\Benefit\Form';
    ...
    {
        return $this->urlBuilder->getUrl('paygcc/benefit/response', ['_secure' => $this->getRequest()->isSecure()]);
    }
    ...
    public function getPayGCCPaymentUrl(\Magento\Quote\Model\Quote $quote)
    {
        ...
        $request['currency_code'] = $quote->getBaseCurrencyCode();
        $request['payment_type'] =  2;
        $request['success_redirect_url'] = $this->getResponseUrl();
        $request['failed_redirect_url'] = $this->getResponseUrl();
